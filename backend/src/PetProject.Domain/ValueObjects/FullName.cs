    ...
    public record FullName
    {
        ...
        public static Result<FullName, Error> Create(string firstName, string lastName, string patronymic) 
        {
            if (string.IsNullOrWhiteSpace(firstName))
                return Errors.General.ValueIsInvalid("first name is null or white space!");

            if (string.IsNullOrWhiteSpace(lastName))
                return Errors.General.ValueIsInvalid("last name is null or white space!");

            if (string.IsNullOrWhiteSpace(patronymic))
                return Errors.General.ValueIsInvalid("patronymic is null or white space!");
